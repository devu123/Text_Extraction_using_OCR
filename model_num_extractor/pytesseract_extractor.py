...
def pytesseract_mn_extractor(img_src: str, logger: Logger, rotate_pic: bool = False) -> Optional[str]:
    img = cv2.imread(img_src)
    content = base_content_ext(img, logger)
    model_numbers = []
    mn = None
    ...
    if rotate_pic:
        for i in range(2):
            rotated_content = rotated_content_ext(img, rotations[i], logger)
            new_mn = None
...
def rotated_content_ext(img, rotation, logger) -> str:
    ...
    txt = pytesseract.image_to_string(adaptive)
...
def base_content_ext(img, logger) -> str:
    img = unskew(img)
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    blurred = cv2.GaussianBlur(gray, (3, 3), 0)
    adaptive = cv2.adaptiveThreshold(blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 15, 4)
